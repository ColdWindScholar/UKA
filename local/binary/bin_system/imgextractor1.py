...
class Extractor(object):
    ...
    def __filter_file_list(self, input_list):
        """
        Фильтрует список, оставляя данные до первого нецифрового или невалидного значения после 4-го элемента.
        """
        filtered_list = []
        for item in input_list:
            parts = item.split()
            filtered_parts = parts[:4]
            if len(parts) > 4:
                for extra_part in parts[4:]:
                    if extra_part.startswith("capabilities=0x") or extra_part.isdigit():
                        filtered_parts.append(extra_part)
                    else:
                        break
            filtered_list.append(" ".join(filtered_parts))
    ...
    def __ext4extractor(self):
            ...
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                ...
                if entry_inode.is_dir:
                    ...
                    try:
                        ...
                        #print('Bad symlink: %s was skipped!!!' %(self.EXTRACT_DIR + entry_inode_path))
                        pass
        ...
        with open(self.OUTPUT_IMAGE_FILE, 'rb') as file:
            ...
            if self.context:  # 11.05.18
                ...
                if self.isSAR:
                    self.context.insert(0, '/' + ' u:object_r:rootfs:s0')
                    self.context.insert(1, '/' + dirr + ' u:object_r:rootfs:s0')
                    self.context.insert(2, '/' + dirr + '/' + ' u:object_r:rootfs:s0')
                    self.context.insert(3, '/' + dirr + '(/.*)? u:object_r:rootfs:s0')
                if not self.isSAR:
                ...
                self.fsconfig_e2fsdroid=self.__filter_file_list(self.fsconfig)
                self.__appendf('\n'.join(self.fsconfig_e2fsdroid), fs_config_file_e2fsdroid) # 20.10.2023
                for l in self.avtoftxt:
                    tmp=re.sub(r' ->.* \d{1,4} \d{1,4} \d{1,4} ', "", l) # 20.10.2023
                    tmp=re.sub(r' \d{1,4} \d{1,4} \d{1,4} ', "", tmp) # 20.10.2023
