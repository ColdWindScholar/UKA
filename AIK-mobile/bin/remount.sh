...
case $0 in
  *.sh) rd="$0";;
     *) rd="$(lsof -p $$ 2>/dev/null | $bb grep -o '/.*remount.sh$')";;
esac;
...
--umount-only() {
  ...
  pid_term="$($bb cat /proc/$$/status | $bb awk '/^PPid:/ { print $2 }')"
 fuser -vm "$loop" &> /data/local/binary/fuser.txt
 
 check_pid="$($bb cat /data/local/binary/fuser.txt | $bb grep -o "$pid_term")"
 if [ -z "$check_pid" ]; then
 fuser -skm "$loop"
  $su -c "$bb umount $aik/ramdisk";
  $bb losetup -d $loop 2>/dev/null || true;
  fi
  $bb rm -f /data/local/binary/fuser.txt
}
